...
export default function Movie({movie, id}) {
  ...
  return (
    <div className='w-[170px] inline-block md:w-[240px] lg:w-[320px] relative p-2'>
      ...
      <div className='absolute top-0 left-0  w-full h-full opacity-0 hover:bg-black/60 hover:opacity-100 flex justify-center items-center'>
         ...
         <p>
            { like ? <FaHeart className='absolute top-6 left-6 text-xl cursor-pointer text-red-500' onClick={() => setLike(false)} /> : <FaRegHeart className='absolute top-6 left-6 text-xl cursor-pointer' onClick={() => setLike(true)}/>}
         </p>
      </div>
    </div>
  )
